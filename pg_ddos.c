    ...
    switch (PQstatus(ddos->conn)) {
        ...
        default: switch (PQconnectPoll(ddos->conn)) {
            ...
            case PGRES_POLLING_FAILED:
                ERROR("PGRES_POLLING_FAILED and %s", PQerrorMessage(ddos->conn));
                if (!strncasecmp(PQerrorMessage(ddos->conn), "could not connect to server", sizeof("could not connect to server") - 1)) ddos_finish(ddos); else ddos_reset(ddos);
                return;
